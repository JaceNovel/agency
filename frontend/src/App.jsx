  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <div className="lg:pl-72">
        ...
        <main className="p-5 lg:p-8"><Routes><Route path="/" element={<Dashboard />} /><Route path="/finance" element={<Finance />} /><Route path="/logement" element={<Housing />} /><Route path="/universites" element={<Universities />} /><Route path="/profil" element={<Profile />} /><Route path="/accompagnement" element={<SupportJourney />} /><Route path="/visa" element={<Visa />} /><Route path="/transport" element={<Transport />} /><Route path="/voyage" element={<Transport />} /><Route path="/esim" element={<Esim />} /><Route path="/messages" element={<Messages />} /><Route path="*" element={<Placeholder />} /></Routes></main>
      </div>
    </div>
  )
}
...
function SupportJourney() {
  return <div className="space-y-8"><section className="relative overflow-hidden rounded-lg bg-white p-12"><img src={avatars.kossi} alt="Conseiller StudyWay" className="absolute bottom-0 right-24 h-80 w-80 rounded-lg object-cover" /><h1 className="max-w-xl text-5xl font-black leading-tight">Accompagnement personnalise</h1><p className="mt-6 max-w-xl text-lg leading-8 text-slate-600">Nous vous accompagnons a chaque etape de votre projet d'etudes a l'etranger. De la preparation du dossier a votre installation, vous n'etes jamais seul.</p><div className="mt-8 flex gap-4"><button className="rounded-lg bg-blue-800 px-8 py-4 font-black text-white">Demarrer mon accompagnement</button><button className="rounded-lg border px-8 py-4 font-black text-blue-800">Voir la video</button></div></section><div className="grid gap-6 xl:grid-cols-[1fr_330px]"><div><h2 className="text-2xl font-black">Nos etapes d'accompagnement</h2><div className="mt-6 grid gap-5 md:grid-cols-5">{['Conseil & Orientation', 'Preparation du dossier', 'Demande de visa', 'Logement & reservation', 'Depart & installation'].map((x, i) => <div className="rounded-lg border bg-white p-6 text-center" key={x}><div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-full bg-blue-50 text-blue-800 font-black">{i + 1}</div><h3 className="font-black">{x}</h3><p className="mt-3 text-sm leading-6 text-slate-600">Suivi clair, humain et personnalise pour avancer sereinement.</p></div>)}</div><div className="mt-6 rounded-lg bg-blue-50 p-5 font-bold text-blue-950">Important: la decision finale d'octroi du visa appartient exclusivement aux autorites consulaires.</div></div><Panel title="Besoin d'aide ?"><List items={['Chat en direct 24h/7j', 'WhatsApp +228 90 12 34 56', 'Email contact@studyway.com', 'Prendre rendez-vous']} /></Panel></div></div>
}

function Visa() {
  const steps = ['Preparation dossier', 'Soumission', 'Rendez-vous', 'Entretien', 'Decision', 'Visa delivre']
  return <div className="grid gap-6 xl:grid-cols-[1fr_380px]"><div className="space-y-6"><section className="grid gap-4 md:grid-cols-4">{[['France court sejour', logos.france], ['France long sejour', logos.france], ['Canada court sejour', logos.canada], ['Canada long sejour', logos.canada]].map(([x, flag]) => <div className="rounded-lg border bg-white p-5"><img src={flag} alt="" className="h-7 w-10 rounded object-cover" /><h3 className="mt-3 font-black">{x}</h3><p className="mt-2 text-sm text-slate-500">Selectionnez ce visa pour commencer votre dossier.</p></div>)}</section><PageTitle title="Mon dossier de visa France" subtitle="Etudes en France - Campus France" /><Panel title="Progression"><div className="grid gap-4 md:grid-cols-6">{steps.map((s, i) => <div className="text-center font-bold" key={s}><div className={`mx-auto mb-2 grid h-12 w-12 place-items-center rounded-full ${i < 2 ? 'bg-emerald-500 text-white' : i === 2 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>{i < 2 ? <CheckCircle2 /> : <CalendarDays />}</div>{s}<div className="text-xs text-slate-500">{i < 2 ? 'Completee' : i === 2 ? 'Programme' : 'A venir'}</div></div>)}</div></Panel><Panel title="Informations du rendez-vous"><div className="grid gap-4 md:grid-cols-4">{['05 Juin 2024', '09:30', 'TLScontact Paris', 'Visa etudiant (VLS-TS)'].map((x) => <div className="font-black" key={x}>{x}</div>)}</div></Panel><Panel title="Etapes de mon dossier"><List items={['Dossier cree - 12 Mai 2024', 'Dossier soumis a Campus France - 20 Mai 2024', 'Rendez-vous pris - 28 Mai 2024', 'Entretien - A venir', 'Decision - En attente', 'Visa delivre - En attente']} /></Panel></div><div className="space-y-5"><Panel title="Mon agent"><div className="flex items-center gap-4"><img src={avatars.koffi} alt="Koffi Adjo" className="h-16 w-16 rounded-full object-cover" /><div><div className="font-black">Koffi Adjo</div><div className="text-slate-500">Agent visa</div></div></div><List items={['+228 90 12 34 56', 'Discuter sur WhatsApp', 'Envoyer un message']} /></Panel><Panel title="Documents requis"><List items={['Passeport - Valide', "Lettre d'admission - Valide", 'Preuve de ressources - Valide', 'Assurance voyage - Valide', 'Releves bancaires - Manquant', 'Certificat de scolarite - Manquant']} /></Panel></div></div>
}
...
function Esim() {
  ...
  return <div className="grid gap-6 xl:grid-cols-[1fr_360px]"><div className="space-y-6"><PageTitle title="eSIM & Forfait" subtitle="Restez connecte des votre arrivee a l'etranger" /><section className="relative rounded-lg bg-blue-50 p-8"><Smartphone size={120} className="absolute left-8 top-10 text-blue-300" /><div className="ml-48"><h2 className="text-2xl font-black">Internet partout dans le monde</h2><List items={['Activation instantanee', 'Pas de carte physique', 'Forfaits flexibles et sans engagement', 'Reseaux fiables et securises']} /></div></section><Panel title="Nos forfaits populaires"><div className="grid gap-4 md:grid-cols-5">{plans.map((p, i) => <div className={`rounded-lg border bg-white p-5 ${i === 2 ? 'border-blue-600 ring-2 ring-blue-100' : ''}`}><img src={i < 3 ? logos.europe : logos.orange} alt="" className="h-9 w-9 rounded-full object-contain" /><h3 className="mt-4 text-xl font-black">{p}</h3><p className="mt-3 text-sm text-slate-500">30 jours<br />Appels illimites<br />SMS illimites</p><div className="mt-6 text-xl font-black">{[15000, 25000, 45000, 15000, 40000][i].toLocaleString('fr-FR')} FCFA</div><button className="mt-5 h-11 w-full rounded-lg bg-blue-600 font-black text-white">Choisir</button></div>)}</div></Panel><Panel title="Mes eSIM"><List items={['Europe 10 Go - Active - 10 Go / 10 Go', 'USA 5 Go - Expiree - 5 Go / 5 Go']} /></Panel></div><div className="space-y-5"><div className="rounded-lg bg-[#061b47] p-6 text-white"><div className="flex justify-between"><h3 className="font-black">Mon eSIM actuelle</h3><span className="rounded-full bg-emerald-500 px-3 py-1 text-sm font-black">Active</span></div><div className="mt-5 flex items-center gap-3 text-xl font-black"><img src={logos.europe} alt="Europe" className="h-10 w-10 rounded-full" />Europe 10 Go</div><p className="text-blue-100">Valable jusqu'au 25 Juin 2024</p><div className="mt-5 h-2 rounded-full bg-white/20"><div className="h-2 w-full rounded-full bg-cyan-300" /></div><button className="mt-6 h-12 w-full rounded-lg bg-white/10 font-black">Voir les details</button></div><Panel title="Comment ca marche ?"><List items={['Choisissez votre forfait', 'Achetez et recevez votre eSIM', 'Scannez le QR code', "Profitez d'internet"]} /></Panel><div className="rounded-lg bg-blue-800 p-6 text-white"><h3 className="text-xl font-black">Parrainez un ami</h3><p className="mt-2">Gagnez 2 000 FCFA pour chaque ami parraine.</p><button className="mt-5 rounded-lg bg-white px-5 py-3 font-black text-blue-800">Parrainer maintenant</button></div></div></div>
}
...
function ProductCard({ title, price, logo }) {
  return <div className="overflow-hidden rounded-lg border bg-white shadow-sm"><div className="relative h-36 bg-gradient-to-br from-blue-100 to-slate-200"><img src={heroImg} alt="" className="h-full w-full object-cover opacity-70" /><button className="absolute right-3 top-3 rounded-full bg-white p-2"><Heart size={18} /></button>{logo && <img src={logo} alt={title} className="absolute bottom-3 left-3 h-10 w-10 rounded-lg bg-white object-contain p-1 shadow" />}</div><div className="p-4"><h3 className="font-black">{title}</h3><p className="mt-1 text-sm text-slate-500">Paris, France</p><div className="mt-4 flex items-center justify-between"><span className="text-xl font-black text-emerald-700">{price}</span><span className="text-sm font-bold text-amber-500">★ 4.8</span></div></div></div>
}

function MiniService({ icon: Icon, title }) {
  return <div className="rounded-lg border border-slate-200 bg-white p-5"><div className="mb-3 grid h-10 w-10 place-items-center rounded-lg bg-blue-50 text-blue-700"><Icon size={20} /></div><h3 className="font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">Service premium StudyWay, suivi par nos conseillers.</p></div>
}
...
function HomeCard({ title, price }) {
  return <div className="overflow-hidden rounded-lg border bg-white shadow-sm"><div className="relative h-44 bg-gradient-to-br from-stone-200 to-blue-100"><img src={heroImg} alt="" className="h-full w-full object-cover opacity-60" /><span className="absolute left-3 top-3 rounded-full bg-emerald-500 px-3 py-1 text-xs font-black text-white">Verifie</span><button className="absolute right-3 top-3 rounded-full bg-white p-2"><Heart size={18} /></button></div><div className="p-4"><h3 className="font-black">{title}</h3><p className="text-sm text-slate-500">Paris, France</p><div className="mt-4 text-xl font-black">{price} € <span className="text-sm font-medium">/ mois</span></div><p className="mt-3 text-sm font-bold text-emerald-600">Disponible maintenant</p></div></div>
}

function UniversityCard({ title, logo, flag }) {
  return <div className="overflow-hidden rounded-lg border bg-white shadow-sm"><div className="relative h-40 bg-blue-100"><img src={heroImg} alt="" className="h-full w-full object-cover opacity-70" /><span className="absolute left-3 top-3 flex items-center gap-2 rounded-full bg-white px-3 py-1 text-sm font-black"><img src={flag} alt="" className="h-4 w-6 rounded object-cover" />Pays</span><button className="absolute right-3 top-3 rounded-full bg-white p-2"><Heart size={18} /></button></div><div className="p-4"><div className="flex items-center gap-3"><img src={logo} alt={title} className="h-11 w-11 rounded-lg object-contain" /><h3 className="font-black">{title}</h3></div><p className="mt-2 text-sm text-slate-500">Paris, France</p><p className="mt-4 text-sm font-bold text-slate-500">QS World Ranking : #63</p><div className="mt-4 flex justify-between text-sm text-slate-500"><span>120 Programmes</span><span>Francais, Anglais</span></div></div></div>
}
...
function Panel({ title, children }) {
  return <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-black">{title}</h2><button className="text-sm font-bold text-blue-600">Voir tout</button></div>{children}</section>
}

function List({ items }) {
  return <div className="divide-y divide-slate-100">{items.map((item) => <div className="flex items-center gap-3 py-4 text-sm font-bold text-slate-700" key={item}><div className="grid h-10 w-10 place-items-center rounded-lg bg-blue-50 text-blue-600"><FileText size={18} /></div>{item}</div>)}</div>
}
