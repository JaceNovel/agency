    ...
    <div className="dashboard-page space-y-7">
      <div className="grid gap-7 xl:grid-cols-[1fr_340px]">
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">{services.map(([label, Icon, sub], index) => <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.045 }} className="service-3d-card rounded-lg border border-slate-200 bg-white p-5 text-center shadow-sm" key={label}><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-blue-50 text-blue-600"><Icon size={28} /></div><div className="mt-4 font-black">{label}</div><p className="mt-2 text-sm leading-6 text-slate-500">{sub}</p></motion.div>)}</div>
        </section>
